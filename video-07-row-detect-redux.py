# video-07-row-detect-redux.py
#
# This value revisits the row detection, and uses a linear regression
# residual to detect rows of holes that lie close to a straight line.
# It also improves the visualization of trace and row data.

#@@@@ deferred
# This version will analyze row-of-hole data from successive frames, and detect
# the sprocket feed holes, allowing for some possibly being missing.
#@@@@ deferred
...
class row_candidate(object):
    ...
    def row_orphaned(self, frnum):
        ...
        #@@TODO: remove this:  return ((rowgap > ROW_FRAME_MAXGAP) and len(self.traces) < 2)
...
def _unused_log_rows(frnum, rows):
...
# Returns displayed video frame
#
def show_video_frame_mask_region_traces_rows(
        frame_label, frame_number, frame_data, rcoords, unused_traces, row_traces, new_rows, old_rows
    ):
    # Display mask frame with centroid data, and return displayed frame value
    frame_show = cv.cvtColor(frame_data, cv.COLOR_GRAY2RGB)
    draw_region_centroids(frame_show, rcoords)
